...
export default function SearchBar({ data }: SearchBarProps) {
	...
	const [filteredResults, setFilteredResults] = useState<SimpleCheatsheet[] | any>([]);
	...
	useEffect(() => {
		...
		if (cleanedTerm === "") {
			...
			const results = data.filter(item => {
				// @ts-ignore
				const alias = aliases[cleanedTerm.toLowerCase()];
				if (item.title.toLowerCase().includes(cleanedTerm.toLowerCase())) {
					return true;
				} else if (alias && alias.includes(item.title.toLowerCase())) {
					return true;
				}
				return false;
			});
				...
				{filteredResults.length > 0 && (
					<div className="dark:bg-[#15151a] dark:border-slate-700 bg-white border border-slate-300 shadow-lg rounded-lg w-full max-w-175 z-20">
						<ul className="max-h-60 overflow-y-auto">
							{filteredResults.map((item: { path: string; title: string }, index: number) => (
								<a key={index} href={item.path} className="text-slate-400 italic">
